...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    ...
    Optional<Schedule> findScheduleByIdWithPlace(@Param("scheduleId") Long scheduleId);

    @Query("select s from Schedule s join s.place p where p.id = :placeId")
    List<Schedule> findSchedulesByPlaceId(@Param("placeId") Long placeId);

    @Query("select s from Schedule s join s.place p where p.placeName = :placeName")
    List<Schedule> findSchedulesByPlaceName(@Param("placeName") String placeName);
